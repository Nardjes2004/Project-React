    ...
    return (
        <div className="flex items-center justify-center max-w-screen-md gap-10 mx-auto">
            <div className="flex flex-col w-full gap-2">
                <img className="bg-gray-100 border border-black" src={product.img} alt={product.title} />

                <div className="grid grid-cols-3 gap-1">
                    {product.images &&
                        product.images.map((image, index) => (
                            <img key={index}  className="bg-gray-100 border border-black" src={image} alt={`Product Image ${index + 1}`} />
                        ))}
                </div>
            </div>
            <div>
